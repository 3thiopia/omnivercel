    ...
    <div className="space-y-4">
          ...
          {mainCategories.map((cat, idx) => (
            ...
            >
                ...
                <AnimatePresence>
                  {selectedParentId === String(cat.id) && (
                    <motion.div
                      ...
                      className="absolute -top-1 -right-1 w-5 h-5 bg-white dark:bg-gray-800 rounded-full border border-emerald-100 dark:border-emerald-900 flex items-center justify-center shadow-md z-10"
                    >
              ...
              <span className={`text-[10px] font-bold transition-colors uppercase tracking-tight max-w-[72px] truncate ${
                selectedParentId === String(cat.id) ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-500 dark:text-gray-400 group-hover:text-emerald-500'
              }`}>
                {cat.name}
              </span>
            </button>
      ...
      {subCategories.length > 0 && (
        <motion.div 
          ...
          className="py-2 overflow-x-auto no-scrollbar border-t border-gray-100 dark:border-gray-800 pt-4 -mx-4 px-4"
        >
            ...
            <button
              ...
              className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all border shadow-sm shrink-0 flex items-center gap-2 ${
                selectedId === selectedParentId
                  ...
                  : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:border-emerald-200 dark:hover:border-emerald-800 hover:text-emerald-600 dark:hover:text-emerald-400'
              }`}
            >
